...
@implementation NSInvocation (TTUtil)

+ (instancetype)tt_invocationWithArgs:(id)target selector:(SEL)selector, ... {
    va_list args;
    va_start(args, selector);
    NSInvocation *inv = [self tt_invocationWithTarget:target selector:selector args:args];
    va_end(args);
    return inv;
}
...
+ (instancetype)tt_invocationWithTarget:(id)target selector:(SEL)selector args:(va_list)args {
    ...
    if (!sig) { [target doesNotRecognizeSelector:selector]; return nil; }
    NSInvocation *inv = [NSInvocation invocationWithMethodSignature:sig];
    if (!inv) { [target doesNotRecognizeSelector:selector]; return nil; }
    [inv setTarget:target];
    [inv setSelector:selector];
    [inv tt_setArgs:args sig:sig];
    return inv;
}

- (void)tt_setArgs:(va_list)args sig:(NSMethodSignature *)sig {
    ...
    for (int index = 2; index < count; index++) {
        ...
        switch (*type) {
            ...
            { // 'float' will be promoted to 'double'.
                ...
                float argf = arg;
                [self setArgument:&argf atIndex:index];
            } break;
...
- (id)tt_argumentAtIndex:(NSUInteger)index {
    NSMethodSignature *sig = self.methodSignature;
    if (index >= sig.numberOfArguments) { return nil; }
    ...
    switch (*type) {
        ...
        default: { // struct / union / SEL / void* / unknown
            ...
            NSValue *value = [NSValue valueWithBytes:valueBytes objCType:type];
            free(valueBytes);
            return value;
        };
    }
...
- (NSArray *)tt_arguments {
    ...
    for (NSUInteger idx = 2; idx < self.methodSignature.numberOfArguments; idx++) {
        [arguments addObject:[self tt_argumentAtIndex:idx] ?: NSNull.null];
    }
